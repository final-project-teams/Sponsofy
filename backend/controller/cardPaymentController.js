...
const validateCardPaymentData = (data) => {
  const { cardNumber, cardHolderName, expirationDate, cvv } = data
...
module.exports = {
  ...
  createCardPaymentForCompany: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: true, message: "Failed to create card payment for company", details: error.message })
    }
  },
  ...
  createCardPaymentForContentCreator: async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ error: false, message: "Card payment created successfully for content creator", cardPayment })
    } catch (error) {
      console.error("Error creating card payment for content creator:", error)
      res
        .status(500)
        .json({ error: true, message: "Failed to create card payment for content creator", details: error.message })
    }
  },
  ...
  updateCardPayment: async (req, res) => {
    try {
      const { id } = req.params
      const { cardNumber, cardHolderName, expirationDate, cvv, companyId, contentCreatorId } = req.body
