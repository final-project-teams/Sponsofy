...
const MOCK_DEALS = [
  ...
  },
  {
    ...
    description: 'Special promotion for the holiday season',
  ...
  },
  {
    ...
    description: 'New products for the spring season',
...
export default function DealsScreen() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      if (useMockData) {
        ...
        console.log("Filtering mock data");
        ...
        if (selectedRank) {
          filtered = filtered.filter(deal => deal.rank === selectedRank);
        }
      ...
      // If search API fails, filter mock data locally
      if (!useMockData) {
        setUseMockData(true);
        let filtered = [...MOCK_DEALS];
  ...
  }, [searchQuery, selectedRank, deals, useMockData]);
  ...
  const handleRankFilter = async (rank) => {
    ...
    try {
      if (useMockData) {
        // Filter mock data locally
        console.log("Filtering mock data by rank:", rank);
        let filtered = [...MOCK_DEALS];
        
        if (rank) {
          filtered = filtered.filter(deal => deal.rank === rank);
        }
      ...
      // If rank filter API fails, filter mock data locally
      if (!useMockData) {
        setUseMockData(true);
        let filtered = [...MOCK_DEALS];
    ...
    return (
      <View style={[styles.container, styles.centeredContent]}>
        ...
        <TouchableOpacity 
          style={styles.button} 
          onPress={() => {
            // Reload the component instead of the window
            setLoading(true);
            setError(null);
            setTimeout(() => {
              const fetchDeals = async () => {
